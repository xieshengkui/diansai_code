...
double current_position_left = 0.0f;   // 左轮累计位置（厘米）
double current_position_right = 0.0f;  // 右轮累计位置（厘米）
double target_position = 0.0f;         // 目标位置（厘米）
...
float position_get_wheel_circumference(void) {
    return 2.0f * PI * WHEEL_RADIUS_CM;
}
...
float position_pd_calc(PID *pid, double current_pos, double target_pos) {
    ...
    // 输出限幅
    if (output > POSITION_OUTPUT_MAX) output = POSITION_OUTPUT_MAX;
    if (output < POSITION_OUTPUT_MIN) output = POSITION_OUTPUT_MIN;
...
void position_update(void) {
    ...
    double delta_left = count_left * position_get_wheel_circumference() / 
                       (ENCODER_LINE_COUNT * ENCODER_REDUCTION_RATIO * ENCODER_MODE_MULTIPLIER);
    double delta_right = count_right * position_get_wheel_circumference() / 
                        (ENCODER_LINE_COUNT * ENCODER_REDUCTION_RATIO * ENCODER_MODE_MULTIPLIER);
...
void position_control(double target_pos) {
    ...
    //    正误差：当前位置 < 目标位置，需要前进
    //    负误差：当前位置 > 目标位置，需要后退
    double pos_err_left = target_pos - current_position_left;
    double pos_err_right = target_pos - current_position_right;
    
    // 3. 判断是否到达目标位置（两轮误差都小于0.5厘米才算到达）
    //    使用fabs只用于判断是否到达，不影响控制方向
    if (fabs(pos_err_left) < 0.5f && fabs(pos_err_right) < 0.5f) {
        // 到达目标：设置目标速度为0，停止电机
        int16_t pwm_left = velocity_pi_calc(&Vpid_left, current_speed_left, 0.0f);
        int16_t pwm_right = velocity_pi_calc(&Vpid_right, current_speed_right, 0.0f);
    ...
    //    左轮根据左轮位置计算目标速度
    double target_speed_left = position_pd_calc(&position_pid_left, current_position_left, target_pos);
    
    //    右轮根据右轮位置计算目标速度
    double target_speed_right = position_pd_calc(&position_pid_right, current_position_right, target_pos);
    
    // 5. 速度环PI控制：根据目标速度计算PWM输出
    int16_t pwm_left = velocity_pi_calc(&Vpid_left, current_speed_left, target_speed_left);
    int16_t pwm_right = velocity_pi_calc(&Vpid_right, current_speed_right, target_speed_right);
