...
void question2_task(void) {
    if(STATE_MACHINE.Q2_State == Q2_State_1){
        ...
        // 状态2：循迹前进
        track_control(150.0f);       // 循迹控制，速度150
        ...
        // 状态3：原地180度转向
        turn_control(150.0f, -179.99f);  // 转向-179.99度（接近180度）
        if(track_read() != 0){            // 检测到赛道
...
void question3_task(void) {
    if(STATE_MACHINE.Q3_State == Q3_State_1){
        ...
        float distance = turn_control_get_distance(150.0f, 60.0f);
        if(distance >= 84.0f){      // 达到目标距离84cm
        ...
        float distance = turn_control_get_distance(150.0f, 120.0f);
        ...
        // 状态5：原地180度转向
        turn_control(150.0f, -179.99f);  // 转向-179.99度
        if(track_read() != 0){            // 检测到赛道
